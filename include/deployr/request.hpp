...
class Request final
{
  ...
  class Channel
  {
    ...
    Channel(const nlohmann::json &channelJs)
    {
      ...
      if (std::find(_producers.begin(), _producers.end(), _consumer) != _producers.end())
        HICR_THROW_LOGIC("Channel '%s' defines '%s' as both consumer and producer. This is not supported\n", _name.c_str(), _consumer.c_str());
